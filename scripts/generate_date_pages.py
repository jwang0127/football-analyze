...
def render(payload: dict[str, Any], styles: dict[str, dict[str, str]]) -> str:
    ...
    for m in payload["matches"]:
        ...
        cards.append(f'''<section class="match" style="--league:{m['leagueStyle']['color']}"><div class="title"><h3>{esc(m['matchNumStr'])} {esc(m['home'])} vs {esc(m['away'])}</h3><span>{esc(m['leagueStyle']['label'])}</span></div><p><b>北京时间：</b>{esc(m['kickoff'])}　<b>公开胜平负赔率：</b>{had.get('home','-')} / {had.get('draw','-')} / {had.get('away','-')}</p><div class="grid"><div><small>胜负平主选</small><strong>{esc(m['directionText'])}</strong></div><div><small>总进球主选</small><strong>{esc(m['totalGoals'])}</strong></div><div><small>主比分</small><strong>{esc(m['mainScore'])}</strong></div><div><small>半全场</small><strong>{esc(HAFU_TEXT[hkey])}</strong></div></div><p><b>胜负平双选：</b>{esc(direction_option_text)}；主选为 {esc(m['directionText'])}。</p><p><b>进球数双选：</b>{esc(goal_option_text)}；前三比分：{esc(score_ranking)}。</p><p><b>综合结论：</b>{esc(m['integratedAnalysis'])}</p><p><a href="analysis_{payload['date']}.html#{esc(m['matchNumStr'])}">查看本场 10 项主客队对照表、证据与结论推导 →</a></p></section>''')
    source_items = "".join(f'<li><a href="{esc(x["url"])}">{esc(x["name"])}</a></li>' for x in payload["sources"])
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#116b62"><title>2026-{label}足球预测</title><style>*{{box-sizing:border-box}}body{{margin:0;background:#eef4f6;color:#17212b;font-family:"Microsoft YaHei",Arial,sans-serif;line-height:1.65}}header,main{{max-width:1180px;margin:auto;padding:24px 16px}}nav a{{margin-right:10px}}h1{{font-size:clamp(30px,5vw,48px)}}.legend span{{display:inline-block;margin:5px;padding:6px 11px;border-left:7px solid var(--c);background:white;border-radius:7px}}.notice,.match,.combo{{background:white;border:1px solid #dce4ea;border-radius:14px;padding:18px;margin:15px 0;box-shadow:0 8px 26px #2336460f}}.notice{{overflow-x:auto}}.match{{border-left:10px solid var(--league);overflow-wrap:anywhere}}.title,.combo h3{{display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap}}.title span{{background:var(--league);color:white;padding:4px 11px;border-radius:99px}}.grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:9px}}.grid div{{background:#f5f8fa;padding:10px;border-radius:8px}}small{{display:block;color:#657482}}strong{{font-size:21px}}.combo{{border-top:6px solid #287d70}}.combo.hafu{{border-top-color:#7a43b6}}.combo.crs{{border-top-color:#b35430}}.combo.ttg{{border-top-color:#355dc5}}.combo.mixed{{border-top-color:#c38b16}}table{{width:100%;border-collapse:collapse}}td{{padding:8px;border-bottom:1px solid #e7ecef}}@media(max-width:700px){{.grid{{grid-template-columns:1fr 1fr}}.combo{{overflow:auto}}}}</style><link rel="stylesheet" href="../assets/site.css"></head><body><header><nav><a href="../index.html">日期首页</a><a href="analysis_{payload['date']}.html">逐项分析表</a><a href="../history/index.html">历史归档</a></nav><h1>{label}足球预测</h1><p>共 {len(payload['matches'])} 场 · 北京时间 · 赔率更新至 {esc(payload['oddsUpdatedAt'])}</p><div class="legend">{legends}</div></header><main>{review_html}{f'<section class="notice"><h2>赛程冲突提示</h2><ul>{warnings}</ul></section>' if warnings else ''}<section class="notice"><h2>逐项分析表</h2><p>每场的硬实力、战术匹配、心理状态 10 项主客队对照、证据、分数和对结果的影响，已单独整理。</p><p><a href="analysis_{payload['date']}.html">打开逐项分析表与最终汇总 →</a></p></section><h2>逐场预测摘要</h2>{''.join(cards)}<section class="notice"><h2>赛程与赔率来源</h2><ul>{source_items}</ul><p>{DISCLAIMER}</p></section></main></body></html>'''


def render_analysis_table(payload: dict[str, Any]) -> str:
    """Render the evidence ledger separately from the short prediction board."""
    label = datetime.strptime(payload["date"], "%Y%m%d").strftime("%m-%d")
    layer_labels = {
        "hardStrength": "第一层｜硬实力",
        "tacticalMatchup": "第二层｜战术匹配",
        "psychologicalState": "第三层｜心理状态",
    }
    match_sections = []
    summary_rows = []
    for m in payload["matches"]:
        three_layer = m.get("threeLayerModel") or {}
        audits = three_layer.get("itemAudit") or {}
        rows = []
        for layer, sides in audits.items():
            home_items, away_items = sides.get("home") or {}, sides.get("away") or {}
            for item in sorted(set(home_items) | set(away_items)):
                home_item, away_item = home_items.get(item, {}), away_items.get(item, {})
                hs, aas = float(home_item.get("score", 50)), float(away_item.get("score", 50))
                diff = hs - aas
                if diff >= 8:
                    impact = f"本项主队 +{diff:.1f} 分，形成主队边际优势"
                elif diff <= -8:
                    impact = f"本项客队 +{abs(diff):.1f} 分，形成客队边际优势"
                else:
                    impact = f"本项差距 {abs(diff):.1f} 分，作为中性或轻微修正"
                status = "已核验" if home_item.get("status") == "verified" or away_item.get("status") == "verified" else "资料不足：中性 50 分"
                rows.append(
                    "<tr>"
                    f"<td>{esc(layer_labels.get(layer, layer))}</td>"
                    f"<td><b>{esc(home_item.get('label') or away_item.get('label') or item)}</b></td>"
                    f"<td><b>{hs:.1f}</b><br>{esc(home_item.get('evidence') or '未检索到可核验资料')}<br><em>{esc(home_item.get('analysis') or '按中性处理')}</em></td>"
                    f"<td><b>{aas:.1f}</b><br>{esc(away_item.get('evidence') or '未检索到可核验资料')}<br><em>{esc(away_item.get('analysis') or '按中性处理')}</em></td>"
                    f"<td>{esc(impact)}<br><small>{esc(status)}｜来源：{esc(home_item.get('source') or away_item.get('source') or '未提供')}</small></td>"
                    "</tr>"
                )
        layer_scores = three_layer.get("layerScores", {})
        layer_line = "｜".join(
            f"{name.replace('hardStrength','硬实力').replace('tacticalMatchup','战术匹配').replace('psychologicalState','心理状态')}：主 {score.get('home',50):.1f} / 客 {score.get('away',50):.1f}"
            for name, score in layer_scores.items()
        ) or "尚未形成三层评分"
        direction_options = " / ".join(f"{x['text']} {x['probability']:.1%}" for x in (m.get("directionOptions") or []))
        goal_options = " / ".join(f"{x['goals']}球 {x['probability']:.1%}" for x in (m.get("goalOptions") or []))
        scores = " / ".join([m["mainScore"], *m.get("backupScores", [])])
        total_text = str(m.get("totalGoals", "-"))
        try:
            over_under = "大 2.5" if int(total_text) >= 3 else "小 2.5"
        except ValueError:
            over_under = "以总进球分布为准"
        match_sections.append(
            f'''<section class="match" id="{esc(m['matchNumStr'])}"><div class="match-head"><div><span class="number">{esc(m['matchNumStr'])}</span><h2>{esc(m['home'])} <i>vs</i> {esc(m['away'])}</h2><p>{esc(m['league'])} · 北京时间 {esc(m['kickoff'])}</p></div><div class="confidence">资料完整度<br><b>{three_layer.get('dataCompleteness', 0):.0%}</b></div></div><p class="layerline">{esc(layer_line)}</p><div class="table-wrap"><table class="audit"><thead><tr><th>分析层</th><th>小项</th><th>{esc(m['home'])}（分数｜证据｜解释）</th><th>{esc(m['away'])}（分数｜证据｜解释）</th><th>本项如何改变结论</th></tr></thead><tbody>{''.join(rows) or '<tr><td colspan="5">本场尚无逐项资料</td></tr>'}</tbody></table></div><div class="conclusion"><b>本场分析结论：</b>{esc(m.get('integratedAnalysis',''))}<br><b>推导：</b>{esc(m.get('analysisBasis',''))}</div></section>'''
        )
        summary_rows.append(
            f"<tr><td>{esc(m['matchNumStr'])}</td><td><b>{esc(m['home'])} vs {esc(m['away'])}</b></td>"
            f"<td>{esc(direction_options)}<br><b>主选：{esc(m['directionText'])}</b><br><small>{esc(m.get('integratedAnalysis',''))}</small></td>"
            f"<td><b>{over_under}</b><br>{esc(goal_options)}</td><td><b>{esc(total_text)} 球</b></td>"
            f"<td>{esc(m.get('halfFullText','-'))}</td><td><b>{esc(scores)}</b></td></tr>"
        )
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{label} 逐项分析表</title><style>:root{{--ink:#17212b;--paper:#f2f6f5;--line:#cbd8d6;--teal:#116b62;--gold:#c88f2b}}*{{box-sizing:border-box}}body{{margin:0;background:var(--paper);color:var(--ink);font:15px/1.6 "Microsoft YaHei",Arial,sans-serif}}header,main{{max-width:1440px;margin:auto;padding:24px 18px}}header{{border-bottom:6px solid var(--teal);background:white;max-width:none;padding-left:max(18px,calc((100% - 1404px)/2));padding-right:max(18px,calc((100% - 1404px)/2))}}a{{color:#075f8e;font-weight:700}}nav a{{margin-right:16px}}h1{{margin:14px 0 4px;font-size:clamp(30px,5vw,52px);letter-spacing:-.04em}}.intro{{max-width:880px;color:#45555b}}.match{{background:#fff;border:1px solid var(--line);margin:22px 0;box-shadow:0 6px 20px #17212b12}}.match-head{{display:flex;justify-content:space-between;gap:16px;padding:18px 20px 10px;border-left:10px solid var(--teal)}}.match-head h2{{margin:3px 0;font-size:25px}}.match-head i{{font-style:normal;color:#819095;font-size:16px}}.match-head p{{margin:0;color:#5c6b70}}.number{{font:700 13px/1 Consolas,monospace;color:var(--teal);letter-spacing:.08em}}.confidence{{text-align:right;color:#5c6b70}}.confidence b{{font-size:24px;color:var(--teal)}}.layerline{{margin:0 20px 16px;padding:9px 12px;background:#e9f3f1;color:#244f4a;font-weight:700}}.table-wrap{{overflow-x:auto}}table{{width:100%;border-collapse:collapse;min-width:1050px}}th{{background:#172f32;color:#fff;text-align:left;font-size:13px;padding:10px;vertical-align:top}}td{{padding:11px 10px;border-top:1px solid var(--line);vertical-align:top}}td:nth-child(1){{color:var(--teal);font-weight:700;width:116px}}td:nth-child(2){{width:110px;background:#f7faf9}}td:nth-child(3),td:nth-child(4){{width:27%}}td:nth-child(5){{width:20%;background:#fff9eb}}em{{display:block;color:#596b70;font-style:normal;margin-top:4px}}small{{color:#5e6b70}}.conclusion{{margin:16px 20px 20px;padding:14px 16px;border-left:4px solid var(--gold);background:#fff9eb}}.final{{background:#fff;border:1px solid var(--line);margin:34px 0;padding:20px;overflow-x:auto}}.final h2{{margin-top:0}}.final table td{{min-width:120px}}.final table td:nth-child(3){{min-width:360px}}@media(max-width:700px){{header,main{{padding:18px 12px}}.match-head{{padding:14px 12px}}.match-head h2{{font-size:21px}}.confidence{{display:none}}.layerline,.conclusion{{margin-left:12px;margin-right:12px}}}}</style></head><body><header><nav><a href="predict_{payload['date']}.html">← 返回预测摘要</a><a href="../index.html">日期首页</a></nav><h1>{label}｜逐项分析表</h1><p class="intro">每场独立列出 10 项。每个分数只在有公开证据时偏离中性 50 分；资料不足会直接写明并保持中性，不将缺失信息伪装成判断。赔率只作次要市场基线。</p></header><main>{''.join(match_sections)}<section class="final"><h2>最终汇总｜胜负平、大小球、进球数、半全场、前三比分</h2><table><thead><tr><th>场次</th><th>对阵</th><th>胜负平双选、主选与原因</th><th>大小球 / 两个进球数</th><th>进球数主选</th><th>半全场</th><th>前三比分</th></tr></thead><tbody>{''.join(summary_rows)}</tbody></table><p>以上仅为公开信息整理后的娱乐分析，不构成任何购彩建议，请理性参考。</p></section></main></body></html>'''
...
def main() -> None:
    ...
    out.joinpath(f"analysis_{args.date}.html").write_text(render_analysis_table(payload), encoding="utf-8")
    if out_root == ROOT:
